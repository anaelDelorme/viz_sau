...
const jsonData = JSON.stringify(bubble_data, null, 2);

// Écrire les données JSON dans un fichier
fs.writeFile('bubble_data.json', jsonData, (err) => {
    if (err) {
        console.error('Erreur lors de l\'écriture du fichier :', err);
        return;
    }
    console.log('Les données ont été écrites dans bubble_data.json avec succès.');
});
